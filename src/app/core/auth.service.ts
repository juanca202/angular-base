...
import { AuthTokenPayload } from 'app/core/models/auth-token-payload';
...
export class AuthService {
  ...
  hasSubscription = computed<boolean>(() => {
    const settings = this.settings(); // Accede a los settings
    return !!(settings && 'subscription' in settings); // Verifica si tiene una suscripción activa
  });
  ...
  public addAuthenticationToken(request: HttpRequest<any>): HttpRequest<any> {
    ...
    if (
      ...
      (environment.auth.refreshTokenUrl &&
        request.url.includes(environment.auth.refreshTokenUrl))
    ) {
      return request;
    }
  ...
  public getToken(): AuthToken | undefined {
    const token: AuthToken =
      this.storageService.get(this.tokenKey, 'local') || '';
  ...
  public getTokenPayload(): AuthTokenPayload | undefined {
    const decodedString: string = window.atob(
      (this.getToken()?.token || '.').split('.')[1]
    );
  ...
  public logout(): Observable<boolean> {
    ...
    location.href =
      window.innerWidth < 1000
        ? `${environment.appPath}/auth`
        : `${environment.appPath}/signin`;
    //location.reload();
    return of(true);
  }
  public signup(data: any, options?: any): Promise<unknown> {
    return lastValueFrom(
      this.restService.post(environment.auth.signupUrl, data, options)
    );
  }
  /**
  ...
  public refreshToken(): Observable<AuthToken> {
    ...
    return this.restService
      .post(url, { refresh_token: token?.refresh_token })
      .pipe(
        tap((token: any) => {
          this.storageService.set(this.tokenKey, token, 'local');
          this.loggedIn.emit(true);
